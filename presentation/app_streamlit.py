...
st.set_page_config(
    page_title="Elite Chess Explorer",
    page_icon="♞", 
    layout="wide")
...
st.title("♟️ Elite Chess Move Explorer")
st.caption(
    "Explore move sequences from elite Lichess games, compare historical "
    "outcomes and discover the most common continuations."
)
...
with hero1:
    with st.container(border=True):
        st.caption("DATASET")
        st.subheader(f"{len(df_prefix):,} sequences")
        st.write("Elite Chess Games")

with hero2:
    with st.container(border=True):
        st.caption("CURRENT DEPTH")
        st.subheader(f"{len(st.session_state.moves)} moves")
        st.write("Moves in the active sequence")

with hero3:
    with st.container(border=True):
        st.caption("ANALYSIS")
        st.subheader("Win and draw rates")
        st.write("Historical outcomes")
...
with col_moves:
    ...
    with st.container(border=True):
        ...
        if st.session_state.moves:
            ...
            st.write("Start by entering a move such as **e2e4**, **d2d4**, **c2c4** or **g1f3**.")
